    ...
    val path: Path = parentPath.let {
        ...
        for (part in JarNameParts.values()) {
            when (part) {
                ...
                JarNameParts.MAPPING -> pathBuilder += if (mappingNamespace != fallbackNamespace) "-${mappingNamespace.namespace}+${fallbackNamespace.namespace}" else "-${mappingNamespace.namespace}"
